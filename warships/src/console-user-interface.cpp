...
void ConsoleUserInterface::LoadGame()
{
    ...
    std::cout << "\n\t Load Game" << std::endl;
    std::cout << " Enter the path to save file (e.g., savefile.txt): ";
    ...
    std::unique_ptr<Game> loadedGame = std::make_unique<Game>(
        battlefield::ManualMode, battlefield::ManualMode);

    loadedGame->GetBattlefields().clear();

    IDatabase& database = this->GetController().GetIDatabase();
    bool success = database.ReadDataFromFile(*loadedGame, ".", fileName, "txt");

    if (!success)
    {
        std::cout << " Error: Failed to load game from file '" << fileName << "'" << std::endl;
        std::cout << " The file may be missing, empty, or corrupted." << std::endl;
        return;
    }

    if (loadedGame->GetBattlefields().empty() || loadedGame->GetBattlefields().size() != 2)
    {
        std::cout << " Error: Invalid game data - expected 2 battlefields" << std::endl;
        return;
    }

    this->GetController().InitGame(std::move(loadedGame));
